...
#   OPENVOXSERVER_HOSTNAME  Hostname of Puppet Server CA, defaults to "puppet"
#   OPENVOXSERVER_PORT      Port of Puppet Server CA, defaults to 8140
...
# returns 1 on failures, 4 for 404, 0 for a 200 OK
httpsreq_insecure() {
    CLIENTFLAGS="-connect ""${OPENVOXSERVER_HOSTNAME}:${OPENVOXSERVER_PORT}"" -ign_eof -quiet $2"
...
HOSTHEADER="Host: ${OPENVOXSERVER_HOSTNAME}"
...
msg "* CA: '${OPENVOXSERVER_HOSTNAME}:${OPENVOXSERVER_PORT}${CA}'"
msg "* SSLDIR: '${SSLDIR}'"
...
msg "Waiting for ${OPENVOXSERVER_HOSTNAME} to be running to generate certificates..."
while ! ca_running; do
    sleep 1
done
...
if ! retry_httpsreq_insecure "$(get "${CA}/certificate/ca")" 10 > "${CACERTFILE}"; then
    error "cannot reach CA host '${OPENVOXSERVER_HOSTNAME}'"
...
if ! retry_httpsreq "$(get "${CA}/certificate_revocation_list/ca")" 10 > "${CRLFILE}"; then
    error "cannot reach CRL host '${OPENVOXSERVER_HOSTNAME}'"
